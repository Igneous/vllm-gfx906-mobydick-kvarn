...
class RocmPlatform(Platform):
    ...
    def check_and_update_config(cls, vllm_config: "VllmConfig") -> None:
        ...
        # (Ported from CudaPlatformBase.check_and_update_config; TURBOQUANT on
        # ROCm did not need this block, so it is added fresh here.)
        ...
        if (
            ...
            # NOTE: on ROCm, cls.get_device_total_memory() uses
            # torch.cuda.get_device_properties, which initializes a HIP context
            # in the parent process (no NVML-style context-free query exists on
            # ROCm). For the single-GPU gfx906 target this is fine; with tensor
            # parallelism it may force `spawn` multiprocessing. Acceptable here.
